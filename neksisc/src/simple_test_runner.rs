...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("========================================");
    println!("NEXUS COMPILER SIMPLE TEST SUITE");
    println!("========================================");
    
    // Read the simple test file
    let test_content = fs::read_to_string("../simple_test.nx")
        .expect("Failed to read simple_test.nx");
    ...
    for opt_level in optimization_levels {
        ...
        match compiler.compile_sequential("simple_test.nx", &test_content) {
            Ok(output) => {
    ...
    println!("========================================");
    println!("Simple test suite completed!");
    println!("========================================");
...
fn extract_test_result(output: &str) -> Option<i32> {
    ...
    for line in &lines {
        if let Some(num) = extract_number(line) {
            if num >= 0 && num <= 10 { // Reasonable range for simple test results
                return Some(num);
            }
        }
    }
